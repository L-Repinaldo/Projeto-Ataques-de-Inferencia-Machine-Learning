...
def run_membership_inference_attack(
    ...
    """
    Membership Inference Attack baseada no comportamento do modelo alvo.
    """

    train_errors = np.abs(y_train_true - y_train_pred)
    test_errors  = np.abs(y_test_true  - y_test_pred)

    # Features do atacante (1D: erro)
    X_attack = np.concatenate([
        train_errors.reshape(-1,1),
        test_errors.reshape(-1,1)
    ])

    # Labels: 1 = membro, 0 = não-membro
    y_attack = np.concatenate([
        np.ones(len(train_errors)),
        np.zeros(len(test_errors))
    ])
